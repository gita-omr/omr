...
OMR::DataType::getVectorSize()
   {
   TR_ASSERT_FATAL(isVector(), "getVectorSize() can only be called on vector type\n");
...
OMR::DataType::maskTypeSize()
   ...
   if (!comp->cg()->usesMaskRegisters())
      ...
      // However, note that, currently, all Mask types(independent on the number of lanes in the corresponding vector type)
...
OMR::DataType::getSize(TR::DataType dt)
   ...
   if (dt.isVector())
      ...
      return dt.getVectorSize();
#else
      static int32_t maskTypeSize = maskTypeSize();
