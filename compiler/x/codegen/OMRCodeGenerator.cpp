...
void OMR::X86::CodeGenerator::doBinaryEncoding()
   ...
   while (cursorInstruction)
      ...
      if (cursorInstruction->isLastWarmInstruction())
         ...
         if (self()->comp()->getOption(TR_TraceCG))
            {
            traceMsg(self()->comp(), "%s warmCodeEnd = %p, after instr = %p starting cold code = %p\n",
                                                           SPLIT_WARM_COLD_STRING,
                                                           getWarmCodeEnd(), cursorInstruction, coldCode);
            }
         ...
         //
         setAccumulatedInstructionLengthError((self()->getBinaryBufferStart() + warmEstimate) + MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE - coldCode);
         }
